...
def buildCoder(shift):
    ...
    for l in range(0, len(lowerCase)):
        coder[lowerCase[l]] = lowerCase[(n+26)%26]
         
    for u in range (0, len(upperCase)):
        coder[upperCase[u]] = upperCase[(u+26)%26]
...
def findBestShift(wordList, text):
    ...
    for shift in range(0,26):
        ...
        for newWord in newTextList:
            if isWord(wordList, newWord):
                wordCounter += 1
        
        #If this number of valid words is more than the largest number of real words found, then:
        if wordCounter > realWords:
            realWords = wordCounter
            bestShift = shift
...
def decryptStory():
    ...
    storyDecripted = applyShift(story, bestShift)
    return storyDecripted
