...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        ...
        #Let's replace all different punctuation of text with space
        #Our tool will be the string created by string.punctuation
        #that is      '!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~'
        punctuation = string.punctuation

        #now we look for any kind of punctuation and replace it
        ...
        #we split all the words in a list text
        text = text.split()

        #print text

        #now, we see if the word (in lowercase) is in that list
...
def filterStories(stories, triggerlist):
    ...
    for text in stories:
        for t in triggerlist:
            if t.evaluate(text):
                if not text in storiesTriggered:
                    storiesTriggered.append(text)


    # This is a placeholder (we're just returning all the stories, with no filtering)
    return storiesTriggered
